...
int16_t call_user(uint8_t funcid UNUSED, uint8_t argc UNUSED,
                  int16_t *argv UNUSED, void *ctx UNUSED) {
#ifndef PROFILE
#ifdef DEBUG
  if (funcid == 0) {
    int i;
...
int main() {
  ...
  fprintf(
      stderr,
      "Executed %d VM instructions in %.2fe%d us CPU time (%.2fe%d MIPS).\n",
